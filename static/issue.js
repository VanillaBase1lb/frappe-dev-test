...
function getStock() {
	fetch('/api/transactions/stock').then(res => res.json()).then(stock => {
		...
		for (let i = 0; i < stock.length; i++) {
			...
			cell0.getElementsByTagName('button')[0].onclick = function() { issueBook(i) };
			// do not put in loop
...
function issueBook(i) {
	...
	const data = { "username": username, "fullname": fullname, "rent": rent, "bookid": bookid };
	console.log(data);
	fetch('/api/transactions/issue', { method: 'POST', headers: { 'Content-Type': 'application/json', }, body: JSON.stringify(data) })
