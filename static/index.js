...
document
  .getElementById("find_book")
  .addEventListener("click", getBooksFromFrappe);
document
  .getElementById("add_book")
  .addEventListener("click", addBooksToLibrary);
...
function getBooksFromFrappe() {
  ...
  fetch("/api/books?" + new URLSearchParams(params), options)
    .then((res) => res.json())
    .then((data) => {
      console.log(data);
      document.books = data;
  ...
  for (let i = 0; i < document.books.length; i++) {
    ...
    if (book.checked) {
      ...
      fetch("/api/books", {
        ...
        body: JSON.stringify({ isbn: book.value }),
      }).then((res) => {
        console.log(res);
      });
    }
